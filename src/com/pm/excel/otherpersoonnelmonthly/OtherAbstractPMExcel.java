...
public abstract class OtherAbstractPMExcel<T extends PersonnelMonthlyBase> {
	IApplyApproveService applyApproveService = null;
	
	public OtherAbstractPMExcel(IApplyApproveService applyApproveService){
	...
	public List<T> getPersonnelMonthly(
			...
			final Map<String, OtherStaff>  staffCostNoMap,
			final Map<String,List<OtherStaff>>  staffCostNameMap
			){
		...
		try{			
			...
			if(baseList != null && !baseList.isEmpty()){
				...
				if(bases != null && bases.size() > 0){
					for(T base : bases){
						OtherStaff staffCost = checkPersonnelMonthlyBase(base,the_month,enumType,staffCostNoMap,staffCostNameMap);
	...
	private OtherStaff checkPersonnelMonthlyBase(
			PersonnelMonthlyBase personnelMonthlyBase,	
			int the_month,
			EnumPersonnelMonthlyType type,
			Map<String,OtherStaff>  staffCostNoMap ,
			Map<String,List<OtherStaff>>  staffCostNameMap){
		...
		if(StringUtils.isEmpty(personnelMonthlyBase.getStaff_no()) && StringUtils.isEmpty(personnelMonthlyBase.getStaff_name())){
			...
			List<OtherStaff> scs = staffCostNameMap.get(personnelMonthlyBase.getStaff_name());
	...
	public abstract boolean checkPersonnelMonthly(T base , OtherStaff staffCost , Map<String,InsuranceGrade> insuranceGrades);
