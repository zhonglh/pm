...
<form id="pagerForm" method="post" action="${webroot }/OtherStaffAction.do?method=lookup1">
	<input type="hidden" name="pageNum" value="${pageNum}" />
	<input type="hidden" name="numPerPage" value="${pageSize}" />
</form>

<div class="pageHeader">
	<form rel="pagerForm"  onsubmit="return dialogSearch(this);" action="${webroot }/OtherStaffAction.do?method=lookup1" method="post">
	<div class="searchBar">
		<ul class="searchContent">
			
		<input type="hidden" name="toDoUrl" value="${toDoUrl}" />
		...
		<tbody>
			...
			<tr target="staff_id" rel="${otherStaff.staff_id }">
				<td>
					...
					<a class="btnSelect" target="ajaxTodo" callback="dialogAjaxDone" href="${webroot }/${toDoUrl }&id=${id }&staff_id=${otherStaff.staff_id}" title="">选择</a>
					</c:if>
				</td>
