...
<form id="pagerForm" method="post" action="${webroot }/OtherPersonnelMonthlyBaseAction.do?method=list">
...
<div class="pageHeader">
	<form rel="pagerForm" onsubmit="return pmSearch(this);" action="${webroot }/OtherPersonnelMonthlyBaseAction.do?method=list" method="post">
	<div class="searchBar">
	...
	<div class="panelBar">
		<ul class="toolBar">
			<c:if test="${operation_insert != null && operation_insert != '' }">
				<li><a class="add" href="${webroot}/OtherPersonnelMonthlyBaseAction.do?method=toFirst" mask="true" width="900" height="400" rel="add_personnelmonthlybase"  target="dialog"><span>添加人事月报</span></a></li>
			</c:if>
			<c:if test="${operation_delete != null && operation_delete != '' }">
				<li><a title="确实要删除这些记录吗?" target="selectedTodo" rel="ids" href="${webroot}/OtherPersonnelMonthlyBaseAction.do?method=deletePersonnelMonthlyBase" class="delete"><span>删除</span></a></li>
		</c:if>
			<c:if test="${operation_update != null && operation_update != '' }">
				<li><a class="edit" href="${webroot}/OtherPersonnelMonthlyBaseAction.do?method=toEditBase&id={sid}" mask="true" width="900"  height="400" rel="update_personnelmonthlybase" target="dialog" warn="请选择一条数据"><span>修改人事月报</span></a></li>
			</c:if>
			<c:if test="${operation_read != null && operation_read != '' }">
				<li><a class="edit" href="${webroot}/OtherPersonnelMonthlyBaseAction.do?method=toViewBase&id={sid}" mask="true" width="900"  height="480" rel="view_personnelmonthlybase" target="dialog" warn="请选择一条数据"><span>查看明细</span></a></li>
			</c:if>	
			<c:if test="${operation_check != null && operation_check != '' }">
				<li><a title="确定已经批量核实了这些单据吗?" target="selectedTodo" rel="ids" href="${webroot}/OtherPersonnelMonthlyBaseAction.do?method=batchVerifyPersonnelMonthlyBase" class="delete"><span>批量核单</span></a></li>
			</c:if>		
			<li class="line">line</li>
			<c:if test="${operation_read != null && operation_read != '' }">
				<li><a class="icon" href="${webroot}/OtherPersonnelMonthlyBaseAction.do?method=export" target="dwzExport" targetType="navTab" title="确定要导出这些记录吗?"><span>导出EXCEL</span></a></li>
			</c:if>	
			<li class="line">line</li>
			<c:if test="${operation_insert != null && operation_insert != '' }">
				<li><a class="add" href="${webroot }/OtherPersonnelMonthlyBaseAction.do?method=toExcel" mask="true" width="850" height="480" rel="add_personnelmonthlybase" target="dialog"><span>导入EXCEL</span></a></li>
			</c:if>
			<c:if test="${operation_insert != null && operation_insert != '' }">	
				<li><a class="icon" href="${webroot }/OtherPersonnelMonthlyBaseAction.do?method=downloadtemplet" target="dwzExport" targetType="navTab" title="确定要导出这模板吗?"><span>下载模板</span></a></li>
			</c:if>		
		</ul>
	</div>
	...
	<table class="list" width="1800" layoutH="60">
		...
		<tbody>
			...
			<tr target="sid" rel="${leaveStaffCost.staff_id }">
				...
				<td>${status1.index+1 }</td>
				<td>${leaveStaffCost.dept_name }</td>
