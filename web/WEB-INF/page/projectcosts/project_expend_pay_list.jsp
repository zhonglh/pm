	...
	<form rel="pagerForm" onsubmit="return navTabSearch(this);" action="${webroot }/ProjectExpendPayAction.do?method=list" method="post">
		<div class="searchBar">
			...
			<ul class="searchContent">
				...
				<li>
					<label>应付金额：</label>
					<b><fmt:formatNumber value="${projectExpendPay.amount }" type="currency" pattern="###,###,##0.00"/></b>

				</li>
				<li>
					<label>累计已付金额：</label>
					<b><fmt:formatNumber value="${projectExpendPay.pay_amount }" type="currency" pattern="###,###,##0.00"/></b>
	...
	<div class="panelBar">
		<ul class="toolBar">
			<c:if test="${operation_insert != null && operation_insert != '' }">
				<li><a class="add" href="${webroot}/ProjectExpendPayAction.do?method=toEdit" mask="true" width="900" height="400" rel="add_projectexpendpay"  target="dialog"><span>添加项目实际支付信息</span></a></li>
			</c:if>
			...
			<c:if test="${operation_update != null && operation_update != '' }">
				<li><a class="edit" href="${webroot}/ProjectExpendPayAction.do?method=toEdit&id={sid}" mask="true" width="900"  height="400" rel="update_projectexpendpay" target="dialog" warn="请选择一条数据"><span>修改项目实际支付信息</span></a></li>
			</c:if>
	...
	<table class="table" width="100%" layoutH="135">
		...
		<tbody>
			...
			<tr target="sid" rel="${projectExpendPay.id }">
				...
				<td>${projectExpendPay.pay_date }</td>
