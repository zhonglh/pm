...
<form id="pagerForm" method="post" action="${webroot }/SalaryGroupAction.do?method=list">
	<input type="hidden" name="pageNum" value="${pageNum}" />
	<input type="hidden" name="numPerPage" value="${pageSize}" />
</form>

<div class="pageHeader">
	<form rel="pagerForm" onsubmit="return navTabSearch(this);" action="${webroot }/SalaryGroupAction.do?method=list" method="post">
	<div class="searchBar">
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			...
			<c:if test="${operation_insert != null && operation_insert != '' }">
				<li><a class="add" href="${webroot}/SalaryGroupAction.do?method=toEditFirst" mask="true" width="900" height="480" rel="add_salaryGroupAction"  target="dialog"><span>添加工资</span></a></li>
				<li><a id="autoadd" title="一键制作月工资" msg="可以一次性的制作所有员工单月份的工资，前提是该项目该月的考勤已经审核通过了，该月份的所有人事月报都审核通过了,该月份的工资还没有制作过。" target="dialog" width="450" height="220" href="${webroot}/SalaryGroupAction.do?method=toAutoAddSalaryGroup" class="add btn-outline-msg"><span>一键添加</span></a></li>
			</c:if>
			...
			<c:if test="${operation_delete != null && operation_delete != '' }">
				<li><a title="确实要删除这些记录吗?" target="selectedTodo" rel="ids" href="${webroot}/SalaryGroupAction.do?method=deleteSalaryGroup" class="delete"><span>删除</span></a></li> 
			</c:if>
			
			<c:if test="${operation_update != null && operation_update != '' }">
				<li><a class="edit" href="${webroot}/SalaryGroupAction.do?method=toEdit&id={sid_salary}" mask="true" width="920"  height="480" rel="update_salaryGroupAction" target="dialog" warn="请选择一条数据"><span>修改工资</span></a></li>
			</c:if>
			
			<c:if test="${operation_update != null && operation_update != '' }">
				<li><a class="edit" href="${webroot}/SalaryGroupAction.do?method=toEdit4Easyui&id={sid_salary}" mask="true" width="920"  height="480" rel="update_salaryGroupAction" target="dialog" warn="请选择一条数据"><span>修改工资</span></a></li>
			</c:if>
			
			
			<c:if test="${operation_read != null && operation_read != '' }">
				<li><a class="edit" href="${webroot}/SalaryGroupAction.do?method=toView&id={sid_salary}" mask="true" width="900"  height="480" rel="view_salaryGroupAction" target="dialog" warn="请选择一条数据"><span>查看明细</span></a></li>
			</c:if>				
			
			<c:if test="${operation_check != null && operation_check != '' }">
				<li><a title="确定已经批量核实了这些单据吗?" target="selectedTodo" rel="ids" href="${webroot}/SalaryGroupAction.do?method=batchVerifySalary" class="delete"><span>批量核单</span></a></li> 
			</c:if>	
			...
			<c:if test="${ totalRows <= 15000}">
				<li><a class="icon" href="${webroot}/SalaryGroupAction.do?method=export" target="dwzExport" targetType="navTab" title="确定要导出这些记录吗?"><span>导出EXCEL</span></a></li>
			</c:if>
			</c:if>
	...
	<table class="table" width="100%" layoutH="135">
		...
		<tbody>
			...
			<tr target="sid_salary" rel="${salary.salary_month }___${salary.project_id }">
				<td>
					<c:if test="${salary.project_salary_number > salary.verify_number }">
						<c:if test="${ 0 == salary.verify_number }">							
							<input name="ids" value="${salary.salary_month }___${salary.project_id }" type="checkbox" /> 
						</c:if>
					</c:if>				
				</td>
				...
				<td>${salary.project_salary_number }</td>
				<td>${salary.build_username }</td>
				<td><fmt:formatDate value="${salary.build_datetime }" pattern="yyyy-MM-dd" /></td>
				<td>
					<c:if test="${salary.project_salary_number == salary.verify_number }"><font color="red">已核单</font></c:if>									
					<c:if test="${salary.project_salary_number > salary.verify_number }">
						<c:if test="${ 0 == salary.verify_number }">
							未核单
						</c:if>
