...
<form id="pagerForm" method="post" action="${webroot }/${action }?method=list">
	<input type="hidden" name="pageNum" value="${pageNum}" />
	<input type="hidden" name="numPerPage" value="${pageSize}" />
</form>

<div class="pageHeader">
	<form rel="pagerForm" onsubmit="return navTabStaisticsSearch(this,${urls });" action="${webroot }/${action }?method=list" method="post">
	<div class="searchBar">
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="icon" href="${webroot }/${action }?method=export" target="dwzExport" targetType="navTab" title="实要导出所有记录吗?"><span>导出EXCEL</span></a></li>
		</ul>
	</div>
		...
		<tbody>
			...
			<tr>
				...
				<td nowarp align="right">
					<a class="add" href="${webroot}/StatisticsDetailAction.do?method=${detailMethod }&project_id=${statistics.project_id }&month1=${param.month1}&month2=${param.month2}" mask="true" width="850" height="520" rel="detail" title="明细"  target="dialog">
					<b><u><c:if test="${statistics.statistics_amount<0 }"><font color="red"></c:if><fmt:formatNumber value="${statistics.statistics_amount }" type="currency" pattern="###,###,##0.00"/><c:if test="${statistics.statistics_amount<0 }"></font></c:if></u></b>
					</a></li>
				</td>	
				<td nowarp align="right"><b><fmt:formatNumber value="${statistics.statistics_rate }" type="currency" pattern="##0.00"/></b></td>								
