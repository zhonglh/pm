...
<form id="pagerForm" method="post" action="${webroot }/ProjectReceivablesStatisticsAction.do?method=list">
	<input type="hidden" name="pageNum" value="${pageNum}" />
	<input type="hidden" name="numPerPage" value="${pageSize}" />
</form>

<div class="pageHeader">
	<form rel="pagerForm" onsubmit="return navTabStaisticsSearch(this,receivables_urls);" action="${webroot }/ProjectReceivablesStatisticsAction.do?method=list" method="post">
	<div class="searchBar">
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="icon" href="${webroot }/ProjectReceivablesStatisticsAction.do?method=export" target="dwzExport" targetType="navTab" title="实要导出所有记录吗?"><span>导出EXCEL</span></a></li>
		</ul>
	</div>
	<table class="table" width="1100" layoutH="162">
		<thead>
			<tr>
				...
				<th width="100">项目管理人</th>
				<th width="100">项目信息来源</th>
				<th width="100">销售额归属部门</th>
				<th width="100">执行额归属部门</th>
				<th width="100">项目归属部门</th>		
			</tr>
		</thead>
		<tbody>
			...
			<tr>
				...
				<td>${statistics.project_name }</td>						
				<td>${statistics.project_no }</td>		
				<td nowarp align="right">
					<a class="add" href="${webroot}/StatisticsDetailAction.do?method=queryReceivablesDetail&project_id=${statistics.project_id }&month1=${param.month1}&month2=${param.month2}" mask="true" width="850" height="520" rel="detail" title="明细"  target="dialog">
					<b><u>
					<c:if test="${statistics.statistics_amount<0 }"><font color="red"></c:if><fmt:formatNumber value="${statistics.statistics_amount }" type="currency" pattern="###,###,##0.00"/><c:if test="${statistics.statistics_amount<0 }"></font></c:if>
					</u></b></a>
				</td>										
				<td>${statistics.sales_username }</td>							
				<td>${statistics.manage_username }</td>							
				<td>${statistics.info_sources_username }</td>							
				<td>${statistics.sales_amount_deptname }</td>							
				<td>${statistics.exec_amount_deptname }</td>							
				<td>${statistics.deptname }</td>				
			</tr>
			</c:forEach>
		</tbody>
	</table>
