 ...
 * 服务器转回navTabId，可以重新载入指定的navTab. statusCode=DWZ.statusCode.ok表示操作成功, 自动关闭当前dialog
 * form提交后返回json数据结构,json格式和navTabAjaxDone一致
 * 用于在dialog上弹出dialog, 保存第二个dialog上的内容， 关闭第二个dialog, 然后刷新第一个dialog
 */
function dialogAjaxDoneEx(json){
	...
	if (json.statusCode == DWZ.statusCode.ok){		
		...
		if (json.rel)	$.pdialog.reload(json.forwardUrl,{"dialogId":json.rel});
 ...
 * @param json
 */
function dialogAjaxDoneByToDo(json){
	DWZ.ajaxDone(json);
	if (json.statusCode == DWZ.statusCode.ok){		
		...
		dwzPageBreak({targetType:"dialog", rel:json.rel, "dialogId":json.rel});
